        ...
        def the_rules
          DocsUI::Section('The rules that make it safe') do
            md <<~MD
              ...
              **A job that raises still clears the pending state.** The markers are
              cleared and the error is *re-raised* so your retry policy sees it. The
              subscription is deliberately **not** torn down on failure — a retry
              must still be able to reach the actor.
